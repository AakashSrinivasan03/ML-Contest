...
train_data = train_data.iloc[0:6000,:]
...
clf = XGBClassifier(max_depth=3,min_child_weight=3,reg_alpha=6, reg_lambda=10,learning_rate=0.1)
clf1=BaggingClassifier(n_estimators=3,base_estimator=clf,max_samples=0.4,n_jobs=-1)
print "htt"
...
y_train_pred_val = clf1.predict(validate_X)
accuracy=metrics.classification_report(validate_Y,y_train_pred_val)
print("accuracy :{}").format(accuracy)
...
sub.to_csv("sub_11.csv", index=False) 
